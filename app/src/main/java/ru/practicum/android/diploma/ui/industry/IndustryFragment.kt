    ...
    private fun showError(errorType: ErrorType) {
        ...
        when (errorType) {
            ErrorType.NoConnection -> {
                ...
                binding.centralImageHolder.setImageResource(R.drawable.nointernet_icon)
            }

            ErrorType.ServerError -> {
                binding.stateTextView.text = getString(R.string.server_error_message)
                binding.centralImageHolder.setImageResource(R.drawable.servererror2_icon)
            }

            else -> {
                binding.stateTextView.text = "getString(R.string.)"
                binding.centralImageHolder.setImageResource(R.drawable.placeholder_nothing)
            }
        }
    }
        ...
        binding.inputEditText.addTextChangedListener(
            afterTextChanged = { text ->
                if (text.isNullOrEmpty()) {
                    binding.searchFrame.setEndIconDrawable(R.drawable.ic_search_24)
                } else {
                    binding.searchFrame.setEndIconDrawable(R.drawable.ic_close_icon_24)
                }
